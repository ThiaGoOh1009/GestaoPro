...
export const EquipamentosPage = ({ addLog }) => {
    ...
    const loadData = useCallback(async () => {
        try {
            const eqData = await backend.getEquipamentos();
            setEquipamentos(eqData);
            const tempData = await backend.getTemporaryEquipamentos();
    ...
    const handleAddItem = async (type) => {
        ...
        const exists = equipamentos.some(e => e.name.toLowerCase() === name.toLowerCase() && e.type === type);
        if (exists) {
            addLog(`Aviso: ${type} "${name}" já existe.`);
            return;
        }
        
        const updated = await backend.saveEquipamento({ type, name });
    ...
    const handleDeleteItem = async (id, name, type) => {
        const updated = await backend.deleteEquipamento(id);
        setEquipamentos(updated);
    ...
    const handleRegisterTemporary = async (originalName) => {
        ...
        const exists = equipamentos.some(e => e.name.toLowerCase() === nameToRegister.toLowerCase());
        
        if (exists) {
            addLog(`Aviso: Equipamento "${nameToRegister}" já existe. Removendo item temporário.`);
        } else {
             await backend.saveEquipamento({ type, name: nameToRegister });
             addLog(`Equipamento temporário "${originalName}" foi cadastrado como "${nameToRegister}".`);
        }
        await backend.deleteTemporaryEquipamento(originalName);
        loadData(); // Reload all data
    };

    const handleDeleteTemporary = async (name) => {
        await backend.deleteTemporaryEquipamento(name);
        ...
        return (
            <div className="bg-gray-800 rounded-xl shadow-lg p-6">
                ...
                <div className="flex space-x-2 mb-4">
                    <input type="text" placeholder={`Novo ${type.toLowerCase()}`} value={newItem[type]} onChange={(e) => handleInputChange(e, type)} onKeyDown={(e) => handleKeyDown(e, type)} className="w-full bg-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    <button onClick={() => handleAddItem(type)} className="flex-shrink-0 px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-500 transition-colors"><PlusIcon className="w-4 h-4" /></button>
                </div>
                <ul className="space-y-2 max-h-80 overflow-y-auto pr-2 sidebar-scroll">
                    {items.length > 0 ? items.map(item => (
                        <li key={item.id} className="flex justify-between items-center bg-gray-700/50 p-3 rounded-lg">
                            ...
                            <button onClick={() => handleDeleteItem(item.id, item.name, item.type)} className="text-red-500 hover:text-red-400 transition-colors" aria-label={`Excluir ${item.name}`}><Trash2Icon className="w-5 h-5" /></button>
                        </li>
                        ...
                        {temporary.map(originalName => (
                            <li key={originalName} className="flex flex-col sm:flex-row justify-between items-center bg-gray-700/50 p-3 rounded-lg gap-3">
                                ...
                                <div className="flex items-center space-x-3 flex-shrink-0">
                                    <button onClick={() => handleRegisterTemporary(originalName)} className="flex items-center text-xs px-2 py-1 bg-green-600 hover:bg-green-500 text-white rounded-md transition-colors" title="Cadastrar Oficialmente"><CheckCheckIcon className="w-4 h-4 mr-1"/> Cadastrar</button>
                                    <button onClick={() => handleDeleteTemporary(originalName)} className="text-red-500 hover:text-red-400 transition-colors" aria-label={`Excluir item temporário ${originalName}`}><Trash2Icon className="w-5 h-5" /></button>
                                </div>
                            </li>
                        ))}
                     </ul>
