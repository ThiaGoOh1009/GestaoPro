...
export const ConfirmacaoRotaModal = ({ isOpen, onClose, onAppend, onCreateNew, regions }) => {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Rota Existente Detectada" maxWidth="max-w-lg">
            ...
            <div className="flex justify-end space-x-3 pt-6 mt-4 border-t border-gray-700">
                ...
                <button type="button" onClick={onCreateNew} className="px-5 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-500 transition-colors">Criar Novas Rotas</button>
                <button type="button" onClick={onAppend} className="px-5 py-2 text-sm font-semibold text-white bg-green-600 rounded-lg hover:bg-green-500 transition-colors">Adicionar às Existentes</button>
            </div>
        </Modal>
    );
};
