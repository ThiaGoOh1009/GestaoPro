...
export const ClientFormModal = ({ isOpen, onClose, onSave, client, fieldSettings = { email: false, phone: false }, addLog, isSaving }) => {
    const initialFormState = { 
        ...
        is_client: true, // Default for client form
        is_supplier: false,
    };
    ...
    useEffect(() => {
        ...
        if (client) {
            const location = client.location && typeof client.location.lat === 'number' ? client.location : initialFormState.location;
            const dataForForm = {
                ...client,
            ...
            setFormData({ ...initialFormState, ...dataForForm, address: { ...initialFormState.address, ...client.address }, location });
            addLog(`Carregando dados para editar cliente: ${client.legal_name}`);
            setMapRefetchKey(k => k + 1);
        } else {
    ...
    const handleSubmit = (e) => {
        ...
        addLog(`Tentativa de salvar cliente: ${dataToSave.legal_name}`);
        onSave(dataToSave);
    };
    ...
    const modalTitle = client ? 'Editar Entidade' : 'Adicionar Cliente';

    return (
        <Modal isOpen={isOpen} onClose={onClose} title={modalTitle} maxWidth="max-w-5xl" closeOnBackdropClick={false} closeOnEscape={false} showCloseButton={false}>
